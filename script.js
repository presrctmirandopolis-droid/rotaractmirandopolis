...
associados.forEach((member, index) => {
  member.slug = member.slug || slugify(member.nome);
  member.id = member.slug || `associado-${index + 1}`;
});
...
function getMemberUrl(member) {
  ...
  url.search = '';
  url.hash = '';
  url.searchParams.set('associado', member.slug);
  return url.toString();
}

function updateMemberUrl(member, replace = false) {
  const url = new URL(window.location.href);
  url.search = '';
  url.hash = '';
  url.searchParams.set('associado', member.slug);
  const method = replace ? 'replaceState' : 'pushState';
  window.history[method]({ associado: member.slug }, '', url);
}

function clearMemberUrl() {
  const url = new URL(window.location.href);
...
function openMember(index, syncUrl = true) {
  ...
  const profileUrl = getMemberUrl(member);
  if (memberProfileUrl) memberProfileUrl.textContent = profileUrl;
  if (copyMemberUrl) {
    copyMemberUrl.dataset.url = profileUrl;
    copyMemberUrl.textContent = 'Copiar URL';
  }

  if (syncUrl) updateMemberUrl(member);
...
function openMemberBySlug(slug) {
  ...
  const index = associados.findIndex(member => member.slug === normalized);
  if (index !== -1) openMember(index, false);
}

function closeMember(clearUrl = true) {
  if (!memberModal) return;
  memberModal.classList.remove('open');
  memberModal.setAttribute('aria-hidden', 'true');
  document.body.classList.remove('modal-open');
  if (clearUrl && new URL(window.location.href).searchParams.has('associado')) {
    clearMemberUrl();
  }
}
...
copyMemberUrl?.addEventListener('click', async () => {
  const url = copyMemberUrl.dataset.url;
  if (!url) return;
  try {
    await navigator.clipboard.writeText(url);
    copyMemberUrl.textContent = 'URL copiada ✓';
    setTimeout(() => { copyMemberUrl.textContent = 'Copiar URL'; }, 1800);
  } catch {
    window.prompt('Copie a URL do perfil:', url);
  }
});

window.addEventListener('popstate', () => {
  const slug = new URL(window.location.href).searchParams.get('associado');
  if (slug) openMemberBySlug(slug);
  else if (memberModal?.classList.contains('open')) closeMember(false);
});
...
const initialMemberSlug = new URL(window.location.href).searchParams.get('associado');
if (initialMemberSlug) openMemberBySlug(initialMemberSlug);
